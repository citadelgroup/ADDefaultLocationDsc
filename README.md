...
* [ADDefaultComputerLocation](#archive): Provides a mechanism to expand an archive (.zip) file to a specific path or remove an expanded archive (.zip) file from a specific path on a target node.
* [ADDefaultUserLocation](#environment): Provides a mechanism to configure and manage environment variables for a machine or process.
